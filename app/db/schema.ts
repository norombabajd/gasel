...
// ---------------------------------------------------------------------------
// Neon Auth schema (managed by Neon — read-only references)
// ---------------------------------------------------------------------------
...
export const matrixItems = pgTable(
  ...
  (table) => [
    foreignKey({
      ...
      foreignColumns: [matrices.id],
      name: 'matrix_items_matrix_fkey',
    }),
  ],
);
...
export const conversations = pgTable(
  'conversations',
  {
    ...
    model: text(),
    content: jsonb(),
    sequenceNum: bigint('sequence_num', { mode: 'number' }),
    createdAt: timestamp('created_at', { withTimezone: true, mode: 'string' }),
  },
  (table) => [
    foreignKey({
      columns: [table.matrix],
      foreignColumns: [matrices.id],
      name: 'conversations_matrix_fkey',
    }),
  ],
);

// User roles — for future role-based access
